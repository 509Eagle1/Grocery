...
function notify(msg, success = true) {
  console.log(msg);
  // Optional: Add toast popup UI later
}
...
function renderChecked() {
  ...
  checkedItems.forEach((item) => {
    ...
    cb.type = "checkbox";
    cb.checked = false; // unchecked copy
...
async function exportToGitHub(showNotify = false) {
  ...
  if (!token) {
    console.log("Cannot export: no token");
    return;
  }
  ...
  try {
    ...
    console.log("Error fetching existing file", e);
  }
...
async function restoreFromGitHub() {
  const token = localStorage.getItem("githubToken");
  if (!token) {
    console.log("Cannot restore: no token");
    return;
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  document
    ...
    .addEventListener("click", () => exportToGitHub(true));
  document
    .getElementById("restoreGitHubBtn")
    .addEventListener("click", restoreFromGitHub);
  document
  ...
  document
    ...
    .addEventListener("click", promptGitHubToken);
  document
    .getElementById("loadTokenFileBtn")
    .addEventListener("click", () =>
