...
async function promptGitHubToken(){
  ...
  if(!token){ token=prompt("⚠️ GitHub token missing! Enter token:")||null; if(token) localStorage.setItem("githubToken",token); }
...
function renderMaster(filter=""){
  ...
  sortedItems.forEach((item,index)=>{
    ...
    dragHandle.className="item-right"; dragHandle.textContent="⇅"; dragHandle.setAttribute("draggable","true");
    dragHandle.addEventListener("dragstart",(e)=>{ e.dataTransfer.setData("text/plain",index); });
    li.appendChild(leftDiv); li.appendChild(dragHandle);
    ...
    li.addEventListener("drop",(e)=>{
      ...
      const temp=groceryItems[draggedIndex];
      groceryItems.splice(draggedIndex,1);
      groceryItems.splice(index,0,temp);
...
function renderChecked(){
  ...
  groceryItems.filter(i=>i.checked).forEach(item=>{
    ...
    cb.addEventListener("change",()=>{
      if(cb.checked){ li.classList.add("checked"); checkedList.appendChild(li); } else li.classList.remove("checked");
    });
  });
}
...
// ===== Clear All Checks =====
function clearAllChecks(){ groceryItems.forEach(i=>i.checked=false); saveData(); renderMaster(); renderChecked(); exportToGitHub(true); }

// ===== GitHub Export/Restore =====
async function exportToGitHub(showNotify=false){ const token=localStorage.getItem("githubToken"); if(!token) return; const content=btoa(JSON.stringify(groceryItems,null,2)); const url=`https://api.github.com/repos/${owner}/${repo}/contents/${path}`; let sha; try{ const getRes=await fetch(url+"?ref="+branch,{headers:{Authorization:`token ${token}`}}); if(getRes.status===200){ const data=await getRes.json(); sha=data.sha;}}catch(e){ console.log("Error fetching existing file", e); }
  try{
...
async function restoreFromGitHub(){
  ...
  try{
    ...
    if(data && data.content){
      ...
      saveData(); renderMaster(); renderChecked(); notify("Restore success ✅");
    } else notify("Restore failed ❌", false);
  }catch(err){ notify("Restore error ❌", false); }
}

// ===== Import / Load Token Files =====
